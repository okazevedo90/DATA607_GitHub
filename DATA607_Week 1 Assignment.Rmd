...
```{r}
med_age_chamber_data = data %>%
  group_by(year, state_region, chamber) %>%
  summarise(med_age = median(age_years), .groups="keep")
ggplot(
  data = med_age_chamber_data,
  mapping = aes(x = year, y = med_age, color = chamber)
) +
  geom_line() +
  facet_wrap(~state_region) +
  labs(
    title = "Median Age of Congress by State Region & Chamber Overtime",
    x = 'Year',
    y = 'Median Age',
    color = 'State Region') + 
  theme_classic()
